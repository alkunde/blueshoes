...
import br.com.mobiletkbrazil.blueshoes.view.MainActivity
import br.com.mobiletkbrazil.blueshoes.data.NavMenuItemsDataBase
...
class NavMenuItemPredicate(
  ...
  override fun canSetStateForKey(
     ...
     * selecionado, então retornaremos false para que o status
     * dele não mude, continue como "selecionado".
     */
    if (!nextState) {
      val lastItemId = NavMenuItemsDataBase(activity).getLastItemId()
      val firstItemLogged = NavMenuItemsDataBase(activity).getFirstItemLoggedId()
      ...
      if ((key <= lastItemId && sizeNavMenuItemsLogged == 0)
        || (key >= firstItemLogged && sizeNavMenuItems == 0)
      ) {
        return false
      }
    }
